...
df_fp <- "~/Dropbox/JSK_SC/_scripts/3_aim1/pub_ema_act_merged.csv"
df <- 
  read_csv(df_fp) %>%
  mutate(
...
asqt3_clean <-
  asqt3 %>%
  rename_at(vars(all_of(names_asqt3_old)), ~names_asqt3_new) %>%
  mutate(
    ELS_ID = factor(ELS_ID)
  ) %>%
...
asqt4_clean <-
  asqt4 %>%
  rename_at(vars(all_of(names_asqt4_old)), ~names_asqt4_new) %>%
  mutate(
    ELS_ID = factor(ELS_ID)
  ) %>%
...
df_cmep_fp <- "~/Dropbox/JSK_SC/_scripts/3_aim1/pub_ema_act_merged.csv"
df_cmep <- 
  read_csv(df_fp) %>%
  mutate(
    ELS_ID = factor(ELS_ID)
...
cmept3_clean <-
  cmept3 %>%
  rename_at(vars(all_of(names_cmept3_old)), ~names_cmept3_new) %>%
  mutate(
    ELS_ID = factor(ELS_ID)
  ) %>%
...
cmept4_clean <-
  cmept4 %>%
  rename_at(vars(all_of(names_cmept4_old)), ~names_cmept4_new) %>%
  mutate(
    ELS_ID = factor(ELS_ID)
  ) %>%
