...
pub_mw_finalcmep <-
  ahc_df %>%
  drop_na(cmep_total)
median(pub_mw_finalcmep$cmep_total) # 27
median(ahc_df$efficiency_mean) #  82.2294
median(ahc_df$sleep_time_hrs_mean) # 6.827473
```
...
ahc_df_cent_2 <-
  ahc_df_cent %>%
  mutate(
    tempo_interval = Age_ahc - Age.T1,
    tempo = (tanner_average_ahc - tanner_average.T1)/tempo_interval
  )
```
...
tempo_dist <- 
  ...
  labs(
    x = "Pubertal Tempo",
    title = "Distribution of Pubertal Tempo"
  ) + 
  theme(
    axis.text = element_text(size = 10, angle = 30, hjust = 1),
    axis.title = element_text(size = 10),
